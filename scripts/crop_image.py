...
def process_loot_images(directory, file_type='*.png', target_width=600):
    ...
    for input_path in files:
        ...
        temp_output = input_path.with_name(f"temp_{input_path.stem}.webp")
        ...
        try:
            ...
            if result.returncode == 0:
                # Если обрезка прошла успешно, удаляем оригинал
                input_path.unlink()
                
                # Формируем правильный финальный путь в той же директории
                final_name = input_path.stem.replace('bogside_outskirts_', '') + '.webp'
                final_path = input_path.with_name(final_name)
                
                # Переименовываем темп
                temp_output.rename(final_path)
                print(f"Готово: {input_path.name} -> {final_path.name} (исходник удален)")
            else:
...
if __name__ == "__main__":
    # Укажи путь к папке. 
    # Судя по твоему коду, это 'public/loot/swamp_forest', 'public/loot/wild_bogs' и т.д.
    target_dir = "public/loot/bogside_outskirts"
